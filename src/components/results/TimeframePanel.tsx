  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-[16px] p-6">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {technical.rules_triggered.map((rule, index) => (
                <span
                  key={index}
                  className="px-3 py-1 bg-blue/20 border border-blue/50 rounded-full text-xs text-blue font-medium"
                >
                  {rule}
                </span>
              ))}
        ...
        {/* Risk Management */}
        {technical.risk && (
          <div className="mb-6 bg-card-hover/50 rounded-lg p-4 border border-border">
      ...
      {meta && (
        <details className="bg-card border border-border rounded-[16px] overflow-hidden">
          ...
          <div className="px-6 pb-4 space-y-3 border-t border-border">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  {Object.entries(meta.summary).map(([key, value]) => (
                    <div key={key} className="bg-card-hover/50 rounded p-2">
            ...
            {meta.usage && (
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(meta.usage).map(([agent, usage]: [string, any]) => (
                    <div key={agent} className="flex items-center justify-between text-xs bg-card-hover/50 rounded p-2">
            ...
            {/* Other Meta */}
            <div className="text-xs text-text-disabled space-y-1 pt-2 border-t border-border">
